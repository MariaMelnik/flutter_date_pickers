...
class WeekPicker extends StatefulWidget {
  ...
  WeekPicker({
    ...
    this.datePickerKeys
       ...
       assert(selectedDate.isBefore(lastDate) || selectedDate.isAtSameMomentAs(lastDate)),
       super(key: key);
...
class _WeekPicker extends StatelessWidget {
  ...
  _WeekPicker({
    ...
    this.selectedPeriodKey
        ...
        assert(!firstDate.isAfter(lastDate)),
        assert(selectedDate.isAfter(firstDate) || selectedDate.isAtSameMomentAs(firstDate)),
        firstDayOfSelectedWeek = _getFirstDayOfWeek(selectedDate, firstDayOfWeekIndex),
        lastDayOfSelectedWeek = _getLastDayOfWeek(selectedDate, firstDayOfWeekIndex),
        super(key: key);
  ...
  static DateTime _getLastDayOfWeek (DateTime day, int firstDayIndex) {
    ...
    lastDayOfWeek = DateTime(lastDayOfWeek.year, lastDayOfWeek.month, lastDayOfWeek.day);
  ...
  bool _isDayOfSelectedWeek (DateTime date) {
    return !(date.isBefore(firstDayOfSelectedWeek) || date.isAfter(lastDayOfSelectedWeek));
  }
  ...
  BoxDecoration _getSelectedDecoration (DateTime date, Color accentColor) {
    ...
    DateTime firstNotDisabledDayOfSelectedWeek = firstDayOfSelectedWeek.isBefore(firstDate)
        ? firstDate
        : firstDayOfSelectedWeek;

    DateTime lastNotDisabledDayOfSelectedWeek = lastDayOfSelectedWeek.isAfter(lastDate)
        ? lastDate
        : lastDayOfSelectedWeek;


    if (DatePickerUtils.sameDate(date, firstNotDisabledDayOfSelectedWeek) && DatePickerUtils.sameDate(date, lastNotDisabledDayOfSelectedWeek)) {
      result = BoxDecoration(
        color: accentColor,
        borderRadius: BorderRadius.all(Radius.circular(10.0)),
      );
    } else if (DatePickerUtils.sameDate(date, firstDayOfSelectedWeek) || DatePickerUtils.sameDate(date, firstDate)) {
      result = BoxDecoration(
        color: accentColor,
        borderRadius: BorderRadius.only(topLeft: Radius.circular(10.0), bottomLeft: Radius.circular(10.0)),
      );
    } else if (DatePickerUtils.sameDate(date, lastDayOfSelectedWeek) || DatePickerUtils.sameDate(date, lastDate)) {
      result = BoxDecoration(
        color: accentColor,
...
// returns new selected period according to tapped date
  DatePeriod _getNewSelectedPeriod (DateTime tappedDay) {
    ...
    DateTime firstDayOfTappedWeek = _getFirstDayOfWeek(tappedDay, firstDayOfWeekIndex);
    DateTime lastDayOfTappedWeek = _getLastDayOfWeek(tappedDay, firstDayOfWeekIndex);
    ...
    newPeriod = DatePeriod(firstNotDisabledDayOfSelectedWeek, lastNotDisabledDayOfSelectedWeek);
    return newPeriod;
  }
...
// returns weather passed day before the beginning of the [firstDay] or after the end of the [lastDay]
  bool _isDisabled(DateTime day) {
    ...
    final DateTime endOfTheLastDay = DateTime(lastDate.year, lastDate.month, lastDate.day, 23, 59, 59, 999);
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; true; i += 1) {
      ...
      if (day < 1) {
        ...
        if (DatePickerUtils.sameDate(dayToBuild, lastDate)) dayToBuild = lastDate;

        final bool disabled = _isDisabled(dayToBuild);
        final bool isSelectedDay = _isDayOfSelectedWeek(DateTime(year, month, day)) && !disabled;
